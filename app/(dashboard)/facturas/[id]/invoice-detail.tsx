...
import { RevertReleaseButton } from "@/components/revert-release-button";
import { InvoiceActivityTimeline } from "@/components/invoice-activity-timeline";
import {
  PurchaseReviewPanel,
  type ChecklistItem,
...
export async function InvoiceDetail({
  ...
  const canRevertRelease =
    isStaff &&
    ["pending", "approved", "rejected"].includes(invoice.status ?? "");
  const canManagePO = isStaff;
  const canConfigureApprovers =
    isStaff &&
    ["in_review", "review_rejected", "pending"].includes(invoice.status ?? "");
  const showReviewPanel = isStaff;
  const approverCount = (approvals ?? []).length;

  const reviewEditable =
    invoice.status === "in_review" || invoice.status === "review_rejected";
  let checklistItems: ChecklistItem[] = [];
  if (showReviewPanel && reviewEditable) {
    const { data: items } = await supabase
      .from("review_checklist_items")
      .select("id, label, description, is_required")
      .eq("is_active", true)
      .order("sort_order", { ascending: true })
      .order("created_at", { ascending: true });
    checklistItems = items ?? [];
  }
  const savedChecklist = Array.isArray(invoice.review_checklist)
  ...
  const duplicates = isStaff
    ? await findDuplicateInvoices({
        supplierNit: invoice.supplier_nit,
  ...
  // Acciones del encabezado (PDF aprobado + eliminar para staff).
  const headerActions = (
    <>
      {finalUrl ? (
      ...
      {canRevertRelease ? (
        <RevertReleaseButton invoiceId={invoice.id} />
      ) : null}
      {isStaff ? (
        <DeleteInvoiceButton
        ...
        <div className="order-2 lg:order-none lg:col-span-2 space-y-4">
          {showReviewPanel ? (
            <PurchaseReviewPanel
              ...
              approverSummary={(approvals ?? []).map((a) => ({
                name: a.approvers?.name ?? "—",
          ...
          <div className="rounded-lg border bg-white overflow-hidden shadow-[0_1px_2px_0_rgb(0_0_0/0.03)]">
            <div className="flex items-center justify-between gap-2 px-4 py-3 border-b">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <span className="inline-flex shrink-0 items-center rounded-full bg-neutral-100 px-2 py-0.5 text-[11px] font-medium text-neutral-600 ring-1 ring-inset ring-neutral-200">
                  {invoice.approval_mode === "sequential"
                    ? "En cascada"
                    : "Independiente"}
                </span>
              ...
              <div className="flex shrink-0 items-center gap-3">
                {canConfigureApprovers && hasApprovals && !reviewEditable ? (
                  <ConfigureApproversDialog
                    invoiceId={invoice.id}
                    supplierName={invoice.supplier_name}
                    currentRequired={invoice.required_approvals}
            ...
            {(approvals ?? []).length === 0 ? (
              <EmptyState
                ...
                action={
                  canConfigureApprovers && !reviewEditable ? (
                    <ConfigureApproversDialog
                      invoiceId={invoice.id}
                      supplierName={invoice.supplier_name}
                      currentRequired={invoice.required_approvals}
          ...
          {/* Historial de actividad (auditoría) — visible para staff. */}
          {isStaff ? <InvoiceActivityTimeline invoiceId={invoice.id} /> : null}
        </div>
